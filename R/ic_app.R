...
ic_app <- function() {
        ...
        column(
          ...
          column(
            ...
            fluidRow(
              ...
              fluidRow(
                column(width = 3, selectInput(inputId = "ic_par_libsource", label = "Data source", choices = list("testdata", "testdata_IDMS", "upload files"), selected = "testdata")),
                column(width = 3, radioButtons(inputId = "ic_par_app_method", label = "Method", choices = c("IR-Delta", "IDMS"))),
              ...
              shiny::div(id = "IDMS_par_section",
                ...
                fluidRow(column(width = 4, numericInput(inputId = "ic_par_IDMS_f", label = "IDMS f-value", value = 0.8876311))),
  ...
  server <- function(input, output, session) {
    ...
    IDMS_data <- reactive({
      ...
      idms <- file_in()
      #validate(need(length(unique(sapply(idms, function(x) { paste0(colnames(x),collapse="") })))==1, "Different columns in files"))
      validate(need(all(c(input$ic_par_mi_col, input$ic_par_si_col) %in% colnames(idms[[1]])), "Select valid columns for MI and SI"))
      # R_observe/R_true
    ...
    output$ic_plot_idms <- renderPlot({
      req(idms_spc(), idms_pks(), ic_table_idms_pre())
      ic_specplot(
        opt = c("overlay_mi", "overlay_pb"),
        xrng = range(sapply(idms_spc(), function(x) {range(MALDIquant::mass(x))})),
        mi_spec = idms_spc(),
        si_spec = NULL,
        xlab = paste0("Time [", "min", "]"),
        ylab = paste0("MF"),
        s_focus = "Sample 1",
        pks = ic_table_idms_pre(),
        mi_pks = idms_pks(),
        cdf = c(0.1, 0.9),
        sel_pk = NULL
      )
    ...
    idms_spc <- shiny::reactiveVal(NULL)
    idms_pks <- shiny::reactiveVal(NULL)
    ic_table_idms_pre <- reactive({
      req(IDMS_data())
      input$ic_par_halfWindowSize
      input$ic_par_peakpicking_SNR
      idms <- IDMS_data()
      spc <- suppressWarnings(lapply(IDMS_data(), function(x) { MALDIquant::createMassSpectrum(mass = x[,input$ic_par_rt_col], intensity = x[,"MF"]) }))
      spc <- MALDIquant_pre_process(spc)
      idms_spc(spc)
      pks <- try(lapply(spc, MALDIquant_peaks), silent = TRUE)
      validate(need(!(inherits(pks, "try-error")), "Can't obtain peaks from IDMS spectra"))
      idms_pks(pks)
      out <- tab_peaks(p = pks, s = spc, mb = c("none","Linear","Russel","Exponential")[3])
      f_value <- log(x = input$ic_par_IDMS_f, base = input$ic_par_mi_amu/input$ic_par_si_amu)
      out[,"f_value"] <- f_value
      out[,"k"] <- IsoCor::mass_bias(mi_amu = input$ic_par_mi_amu, si_amu = input$ic_par_si_amu, method = "Russel", f_value = f_value)
      for (i in 1:length(idms)) {
        idms[[i]][,"MF_cor"] <- spc[[i]]@intensity
        idms[[i]][,"MF_dt"] <- idms[[i]][,"MF_cor"] * c(diff(idms[[i]][,input$ic_par_rt_col]), 0)
      }
    ...
    MALDIquant_pre_process <- function(x) {
      if (isolate(input$ic_par_halfWindowSize)>0 & input$ic_par_app_method=="IDMS") {
        # smoothing is omitted from re processing for IsoCor workflow but used in IDMS workflow
        x <- smoothIntensity(
          object = x,
    ...
    ic_table_peaks_pre <- reactive({
      ...
      out <- tab_peaks(p = ic_mi_peaks(), s = ic_mi_spectra(), mb = isolate(current_mb_method()))
        ...
        callbackR = function(value) {
          if (value) {
            ...
            if (prod(dim(tmp))==nrow(tab) && all(is.finite(tmp))) {
              ...
              if (prod(dim(tmp))==1 && all(is.finite(tmp))) tab[, "f_value"] <- rep(tmp[1,1], nrow(tab))
            }
    ...
    # idms table
    output$ic_table_IDMS <- renderDT({
      message("output$ic_idms_table")
      style_tab_idms(data = ic_table_idms_pre(), sh = screen_height())
    })
